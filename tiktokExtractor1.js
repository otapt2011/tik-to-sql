...
// WITH DATA NORMALIZATION INTEGRATED AND PROPER SCHEMA HANDLING
var TikTokExtractor = (function() {
    ...
    var config = {
        ...
        // Normalization settings - AUTOMATICALLY ENABLED
        normalizeData: true, // Auto-enabled
        normalizationMode: 'quick' // Default: quick normalization
    };
    ...
    // NEW: Store pre-normalization schema
    var tableSchemas = {};
    var sqlStatements = [];
    ...
    var dataNormalizer = null;
    function initializeNormalizer() {
        if (typeof DataNormalizerCore !== 'undefined') {
            dataNormalizer = new DataNormalizerCore();
        } else if (typeof window !== 'undefined' && window.DataNormalizerCore) {
            dataNormalizer = new window.DataNormalizerCore();
        } else {
            console.warn('DataNormalizerCore not found. Data normalization will be skipped.');
            config.normalizeData = false; // Disable if normalizer not available
        }
    }
    ...
    // Normalize extracted data AUTOMATICALLY
    function normalizeExtractedData() {
        if (!config.normalizeData || !dataNormalizer) {
            console.log('Data normalization is disabled');
            return; // Skip normalization
        }
        
        console.log('Starting automatic data normalization (quick mode)...');
        
        // Update progress
        notifyProgress(
            extractedData.progress.processed,
            extractedData.progress.total,
            95,
            'Normalizing Data',
            'Applying quick normalization'
        );
        
        try {
            // Store original statistics before normalization
            extractedData.normalization.originalStats = { ...extractedData.statistics };
            extractedData.normalization.applied = true;
            extractedData.normalization.mode = config.normalizationMode;
            
            // Count total records before normalization
            let totalBefore = 0;
            for (const table in extractedData.tables) {
                if (Array.isArray(extractedData.tables[table])) {
                    totalBefore += extractedData.tables[table].length;
                }
            }
            
            // Apply quick normalization
            const normalizedResult = dataNormalizer.quickProcess({
                tables: extractedData.tables
            });
            
            // Replace tables with normalized data
            if (normalizedResult && normalizedResult.tables) {
                extractedData.tables = normalizedResult.tables;
                
                // Update statistics
                for (const tableName in extractedData.tables) {
                    if (Array.isArray(extractedData.tables[tableName])) {
                        extractedData.statistics[tableName] = extractedData.tables[tableName].length;
                    }
                }
                
                // Count total records after normalization
                let totalAfter = 0;
                for (const table in extractedData.tables) {
                    if (Array.isArray(extractedData.tables[table])) {
                        totalAfter += extractedData.tables[table].length;
                    }
                }
                
                extractedData.normalization.normalizedStats = {
                    totalBefore: totalBefore,
                    totalAfter: totalAfter,
                    tablesNormalized: Object.keys(extractedData.tables).length
                };
                
                console.log(`Data normalization completed! Tables: ${Object.keys(extractedData.tables).length}, Records: ${totalBefore} → ${totalAfter}`);
                
                // Add normalization info to warnings for user visibility
                extractedData.warnings.push({
                    type: 'normalization_info',
                    message: `Data normalized using ${config.normalizationMode} mode. Processed ${totalBefore} records.`,
            ...
            // Log warning but continue without normalization
            extractedData.warnings.push({
            ...
            extractedData.normalization.applied = false;
            extractedData.normalization.mode = 'failed';
    ...
    function extractData(jsonData, options) {
        ...
        try {
            ...
            // Step 3: Extract all data with date handling (stores pre-normalization schema)
            extractAllMappingsData(jsonData, totalItems);
            
            // Step 4: Generate SQL schema BEFORE normalization
            generateTableSchema();
            
            // Step 5: AUTOMATIC NORMALIZATION (quick mode)
            if (config.normalizeData) {
                normalizeExtractedData();
            }
            
            // Step 6: Validate dates if enabled
            if (config.validateDates) {
                validateExtractedDates();
            }
            
            // Step 7: Generate SQL INSERT statements with proper user_id and triggers
            ...
            return {
                ...
                normalization: extractedData.normalization
            };
        } catch (error) {
            isProcessing = false;
    ...
    // Extract data from all mappings and store pre-normalization schema
    function extractAllMappingsData(jsonData, totalItems) {
        ...
        for (const path in mappings) {
            ...
            try {
                ...
                if (extracted.length > 0) {
                    ...
                    // Store FIRST ROW for schema detection (pre-normalization)
                    if (!tableSchemas[tableName] && rowsWithUserId.length > 0) {
                        tableSchemas[tableName] = {
                            sampleRow: rowsWithUserId[0],
                            mapping: mapping
                        };
                    }
        ...
        notifyProgress(processedItems, totalItems, 100, 'Extraction Complete', '');
    }
    
    // Generate table schema from pre-normalization data
    function generateTableSchema() {
        console.log('Generating table schema from pre-normalization data...');
...
CREATE TABLE IF NOT EXISTS users (
    ...
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        ...
        // Create tables for all known tables using PRE-NORMALIZATION schema
        for (const tableName of allTableNames) {
            ...
            // Use PRE-NORMALIZATION schema if available
            if (tableSchemas[tableName] && tableSchemas[tableName].sampleRow) {
                const sampleRow = tableSchemas[tableName].sampleRow;
                const mappingConfig = tableSchemas[tableName].mapping;
                ...
                // Fallback to mapping configuration
                const mappings = mapper.getAllMappings();
                for (const path in mappings) {
...
CREATE INDEX IF NOT EXISTS idx_searches_date ON searches(search_date);`);
        ...
        if (config.generateTriggers) {
            ...
            sqlStatements.push(...triggerStatements);
        }
        
        // Add report views
        const viewStatements = generateReportViews();
        sqlStatements.push(...viewStatements);
        
        console.log(`Generated schema for ${allTableNames.length} tables`);
    }
...
ORDER BY total_issues DESC;`);
    ...
    // Update table statistics to include ALL tables
    ...
    function generateSQLStatements() {
        // Don't reset sqlStatements - keep the schema we already generated
        ...
        // Generate INSERT statements for all tables with NORMALIZED data
    ...
    function generateInsertSQL(tableName, row) {
        ...
        // Get date and numeric fields for this table from PRE-NORMALIZATION schema
        let dateFields = [];
        let numericFields = [];
        if (tableSchemas[tableName] && tableSchemas[tableName].mapping) {
            const mapping = tableSchemas[tableName].mapping;
            if (mapping.dateFields) {
                dateFields = mapping.dateFields;
            }
            if (mapping.numericFields) {
                numericFields = mapping.numericFields;
            }
        }
        
        // Add all columns from the NORMALIZED row
        for (const column in row) {
            if (row.hasOwnProperty(column) && 
                ...
                // Check if this column might have been normalized
                let value = row[column];
                values.push(formatSQLValue(column, value, dateFields, numericFields));
            }
        }
    ...
    function downloadSQL(filename = 'tiktok_data.sql') {
        ...
        const normalizationInfo = extractedData.normalization.applied ? 
            `-- Data Normalization: APPLIED (${extractedData.normalization.mode} mode)` : 
...
${normalizationInfo}
-- Schema: Generated from PRE-NORMALIZATION data
-- Data: NORMALIZED with quick mode
...
`;
    ...
    // Normalization control methods
    function enableNormalization(mode = 'quick') {
        config.normalizeData = true;
        config.normalizationMode = mode;
        console.log(`Normalization enabled: ${mode} mode`);
    }
    ...
    function setNormalizationMode(mode) {
        if (['quick', 'preserve', 'compress', 'custom'].includes(mode)) {
            config.normalizationMode = mode;
            console.log(`Normalization mode set to: ${mode}`);
        }
    }
    
    function getNormalizationStatus() {
        return {
            enabled: config.normalizeData,
            mode: config.normalizationMode,
            applied: extractedData.normalization.applied,
            stats: extractedData.normalization
        };
    }
    ...
    return {
        ...
        getNormalizationStatus: getNormalizationStatus,
        // Configuration
