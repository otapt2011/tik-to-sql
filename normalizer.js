...
class DataNormalizerCore {
    ...
    // Extract video hash from URL (handles both types)
    ...
    extractVideoId(text) {
        return this.extractVideoHash(text);
    }
    
    // Extract media filename from bracketed URL
    extractMediaFilename(text) {
        if (!text || typeof text !== 'string') return text || '';
        const mediaRegex = /\[https?:\/\/[^\]]+\/([^\/?\]]+)(?:\?[^\]]*)?\]/g;
        return text.replace(mediaRegex, '$1');
    }
    ...
    // NEW: Extract filename from effect link (similar to sound links)
    ...
    // Process a single message with options
    processMessage(text, options = {}) {
        ...
        // Extract TikTok IDs
        if (extractVideoId) {
            processed = this.extractVideoId(processed);
        }
        
        // Extract media filenames
        if (extractMediaFilename) {
            processed = this.extractMediaFilename(processed);
        }
    ...
    // NEW: Process effect links - extract filename
    ...
    // Process an array of items - SIMPLIFIED version without stats
